...
class WakeWordValidator(Validator):
    ...
    def __init__(self, validators=None):
        ...
        self.validators = validators or {
            'isWakeWord': lambda val: type(val) == str and (val == 'true' or val == 'false'),
            'noiseLevel': lambda level: type(level) == str and level in 'qml' and len(level) == 1,
            'tone': lambda tone: type(tone) == str,
            'location': lambda location: type(location) == str,
            'gender': lambda gender: type(gender) == str and gender in 'mf' and len(gender) == 1,
